  ...
  Object.values(players).forEach((player) => {
    ...
    if(font){
      if(!playerMesh.getObjectByName('nickname')){
        ...
        mesh = new THREE.Mesh(
          new THREE.TextGeometry(player.nickname, {font: font, size: 10, height: 1}), nicknameMaterial,
        );
        mesh.name = 'nickname';
        playerMesh.add(mesh);
      ...
      }
      {
        ...
        if(!mesh){
          ...
          mesh = new THREE.Mesh(
            new THREE.TextGeometry('*'.repeat(player.health), {font: font, size: 10, height: 1}), textMaterial,
          );
          mesh.name = 'health';
